...
bool KannalaBrandt8::ReconstructWithTwoViews(const std::vector<cv::KeyPoint> &vKeys1, const std::vector<cv::KeyPoint> &vKeys2, const std::vector<int> &vMatches12, Sophus::SE3f &T21, std::vector<cv::Point3f> &vP3D, std::vector<bool> &vbTriangulated) const {
  if (!tvr) {
...
bool KannalaBrandt8::epipolarConstrain(const std::shared_ptr<const GeometricCamera> &pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const Eigen::Matrix3f &R12, const Eigen::Vector3f &t12, const float sigmaLevel, const float unc) const {
  Eigen::Vector3f p3D;
  return this->TriangulateMatches(pCamera2, kp1, kp2, R12, t12, sigmaLevel, unc, p3D) > 0.0001f;
}

float KannalaBrandt8::TriangulateMatches (const std::shared_ptr<const GeometricCamera> &pCamera2, const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const Eigen::Matrix3f &R12, const Eigen::Vector3f &t12, const float sigmaLevel, const float unc, Eigen::Vector3f &p3D) const {
...
std::ostream &operator<<(std::ostream &os, const KannalaBrandt8 &kb) {
  os << kb.mvParameters[0] << " " << kb.mvParameters[1] << " " << kb.mvParameters[2] << " " << kb.mvParameters[3] << " " << kb.mvParameters[4] << " " << kb.mvParameters[5] << " " << kb.mvParameters[6] << " " << kb.mvParameters[7];
  return os;
}
...
void KannalaBrandt8::Triangulate(const cv::Point2f &p1, const cv::Point2f &p2, const Eigen::Matrix<float, 3, 4> &Tcw1, const Eigen::Matrix<float, 3, 4> &Tcw2, Eigen::Vector3f &x3D) const {
...
bool KannalaBrandt8::IsEqual(const std::shared_ptr<GeometricCamera> &pCam) const {
...
bool KannalaBrandt8::IsEqual(const std::shared_ptr<const GeometricCamera> &pCam) const {
  if (pCam->GetType() != GeometricCamera::CAM_FISHEYE) return false;

  std::shared_ptr<const KannalaBrandt8> pKBCam = std::static_pointer_cast<const KannalaBrandt8>(pCam);
...
const std::vector<int> &KannalaBrandt8::getLappingArea() const { return mvLappingArea; }
void KannalaBrandt8::scaleLappingArea(float f){
  mvLappingArea[0] *= f;
  mvLappingArea[1] *= f;
}
